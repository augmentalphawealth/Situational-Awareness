...
# ✅ FIX 1: Reduced chunk size to 25 to match the successful EOD rate limits
chunk_size = 25 
...
for chunk in chunks:
    # ✅ FIX 2: Increased retry attempts to 5
    ...
    # ✅ FIX 3: Increased delay between requests to 0.4s to prevent Angel One silent firewall bans
    time.sleep(0.4) 
...
# 4. Aggregations (Only need to calculate the final dashboard row)
...
# Also update the basic live counter for the UI Banner
live_advances = final_summary.iloc[-1]['Advances']
live_declines = final_summary.iloc[-1]['Declines']
live_total = final_summary.iloc[-1]['Total_Universe'] # ✅ FIX 4: Explicitly pass the Total Universe to the live tracker
