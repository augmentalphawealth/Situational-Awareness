...
def kite_call(fetcher, label: str) -> object | None:
    result = fetch_with_backoff(fetcher)
    if result is None:
        print(f"⚠️ {label} failed after shared Kite retry/backoff handling.")
    return result
...
def fetch_live_quotes(
    ...
    def fetch_chunk(chunk: list[str], chunk_label: str) -> None:
        ...
        if response is None:
            if len(chunk) == 1:
                failures.append(
                    {"Symbol": chunk[0], "Reason": "quote_request_failed"}
                )
                return
        ...
        for symbol in sorted(set(chunk) - returned_symbols):
            failures.append(
                {"Symbol": symbol, "Reason": "missing_from_quote_response"}
            )
    ...
    valid_rows = live.apply(is_valid_live_ohlcv, axis=1)
    invalid_symbols = set(live.loc[~valid_rows, "Symbol"].map(clean_symbol))
    ...
    existing_failure_symbols = {
        record.get("Symbol") for record in failures
    }
...
def prepare_stock_data(
    ...
    df["Active_Universe"] = (
        (mature_valid | new_valid) & (df["Volume"] > 0)
    )
    ...
    prior_tight = (
        group["VCP_Tightness"].shift(1).astype(float).fillna(0).astype(bool)
    )
    df["Mature_Breakout"] = (
        (df["Prior_History_Days"] >= 20)
    ...
    df["Is_Breakout_3d_ago"] = (
        group["Is_Breakout"].shift(3).astype(float).fillna(0).astype(bool)
    )
...
def intraday_volume_points(volume_ratio: pd.Series) -> pd.Series:
    return pd.Series(
        np.select(
            [
                volume_ratio < 0.80,
                (volume_ratio >= 0.80) & (volume_ratio < 1.00),
                (volume_ratio >= 1.00) & (volume_ratio < 1.20),
                (volume_ratio >= 1.20) & (volume_ratio < 1.50),
                (volume_ratio >= 1.50) & (volume_ratio < 2.00),
                volume_ratio >= 2.00,
            ],
            [0, 2, 4, 6, 8, 10],
            default=0,
...
def calculate_aggregate(stock: pd.DataFrame) -> pd.DataFrame:
    ...
    result["Rolling_3D_Up_4"] = result["Up_4_Count"].rolling(3, min_periods=1).sum()
    result["Rolling_3D_Down_4"] = result["Down_4_Count"].rolling(3, min_periods=1).sum()
    ...
    result["T3_Breakouts_5D"] = result["T3_Breakouts"].astype(float).rolling(5, min_periods=1).sum()
    result["T3_Wins_5D"] = result["T3_Wins"].astype(float).rolling(5, min_periods=1).sum()
    t3_breakouts_5d = result["T3_Breakouts_5D"]
    t3_wins_5d = result["T3_Wins_5D"]
    smoothed = (t3_wins_5d + 5) / (t3_breakouts_5d + 10)
    c2 = (
        np.clip((smoothed - 0.45) / 0.15, 0, 1)
        * 25
        * np.clip(t3_breakouts_5d / 10, 0, 1)
    )
    ...
    result["Intraday_Volume_Points"] = intraday_volume_points(result["Volume_Ratio"])
    rank_hl = result["Net_52W_High_Low"].rolling(126, min_periods=1).rank(pct=True)
    c4 = result["Intraday_Volume_Points"] + np.where(
...
def merge_live_today_with_eod_history(
    ...
    intraday_aggregate["Date"] = pd.to_datetime(
        intraday_aggregate["Date"], errors="coerce"
    ).dt.normalize()
    intraday_aggregate = intraday_aggregate.dropna(subset=["Date"])
    ...
    if not EOD_AGGREGATE_FILE.exists():
        ...
        return (
            intraday_aggregate.sort_values("Date")
    ...
    eod_aggregate["Date"] = pd.to_datetime(
        eod_aggregate["Date"], errors="coerce"
    ).dt.normalize()
    eod_aggregate = eod_aggregate.dropna(subset=["Date"])
    eod_history = eod_aggregate[eod_aggregate["Date"] < TODAY].copy()
    ...
    return (
        live_aggregate.sort_values("Date")
        .drop_duplicates("Date", keep="last")
        .reset_index(drop=True)
    )
...
def main() -> None:
    ...
    missing_columns = set(REQUIRED_COLUMNS).difference(historical.columns)
    if missing_columns:
        fail("Master data missing columns: " f"{sorted(missing_columns)}")
    ...
    print(
        "Intraday eligible symbols with usable history: "
    ...
    audit_records.extend(
        {"Date": TODAY, "Reason": "quote_fetch_detail", **failure}
    ...
    live_aggregate = merge_live_today_with_eod_history(calculated_aggregate)
    today_row = live_aggregate[live_aggregate["Date"] == TODAY]
    if today_row.empty:
    ...
    write_json(
        AUDIT_DIR / f"intraday_run_{audit_stamp}.json",
        {
            ...
            "composite_score": int(current["Composite_Score"]),
            "volume_ratio": (
                float(current["Volume_Ratio"])
                if pd.notna(current["Volume_Ratio"])
                else None
            ),
            "intraday_volume_points": int(current["Intraday_Volume_Points"]),
            "daily_t3_breakouts": int(current["T3_Breakouts"]),
            "daily_t3_wins": int(current["T3_Wins"]),
            "t3_breakouts_5d_for_score": int(current["T3_Breakouts_5D"]),
            "t3_wins_5d_for_score": int(current["T3_Wins_5D"]),
        },
    )

    print(f"✅ Live aggregate generated: {TODAY.date()}")
    print(f"✅ Composite Score: {int(current['Composite_Score'])}")
    print(
        f"✅ Volume Ratio: {current['Volume_Ratio']:.2f}"
        if pd.notna(current["Volume_Ratio"])
    ...
    print(
        f"✅ Daily T+3: {int(current['T3_Wins'])}/"
