    ...
    for start in range(0, len(instruments), 200):
        ...
        for attempt in range(5):
            try:
                ...
                print(f"⚠️ Quote attempt {attempt + 1}/5 failed: {type(exc).__name__}")
    ...
    for period in [20, 50, 200]:
        df[f"EMA_{period}"] = group["Close"].transform(
            lambda x, p=period: x.ewm(
                span=p, adjust=False, min_periods=1
            ).mean()
        )
...
def main() -> None:
    print("Connecting to Zerodha API for authoritative live analytics...")
    ...
    symbols = sorted(master["Symbol"].dropna().astype(str).str.strip().unique())
    kite = connect_to_kite()
    live = fetch_live_quotes(kite, symbols)
    stock_data = prepare_stock_data(master, live)
    aggregate = calculate_aggregate(stock_data)
    ...
    atomic_write(
        today_row[["Date", "Advances", "Declines", "Unchanged", "Total_Universe"]].assign(
