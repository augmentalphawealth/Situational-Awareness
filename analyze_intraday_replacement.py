...
def fetch_live_quotes(kite: KiteConnect, symbols: list[str]) -> pd.DataFrame:
    ...
    for start in range(0, len(instruments), 200):
        chunk = instruments[start:start + 200]
        response = None
        ...
        for attempt in range(5):
            try:
                ...
                print(f"⚠️ Quote attempt failed: {type(exc).__name__}: {exc}")
...
def prepare_stock_data(master: pd.DataFrame, live: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Prior_Turnover_20D_Avg"] = group["Daily_Turnover"].transform(lambda x: x.shift(1).rolling(20, min_periods=1).mean())
    df["Active_Universe"] = (((df["Prior_History_Days"] >= 20) & (df["Prior_Turnover_20D_Avg"] >= 50_000_000)) | ((df["Prior_History_Days"] >= 1) & (df["Prior_History_Days"] < 20))) & (df["Volume"] > 0)

    df["Turnover_45d_Avg"] = group["Daily_Turnover"].transform(lambda x: x.shift(1).rolling(45, min_periods=1).mean())
    ...
    for period in [20, 50, 200]:
        df[f"EMA_{period}"] = group["Close"].transform(lambda x, p=period: x.ewm(span=p, adjust=False, min_periods=1).mean())
        df[f"Valid_{period}_EMA"] = df["Active_Universe"] & (df["Prior_History_Days"] >= period)
        df[f"Above_{period}_EMA"] = df[f"Valid_{period}_EMA"] & (df["Close"] > df[f"EMA_{period}"])
    ...
    df["Gainer"] = df["Active_Universe"] & df["Has_Prior_Close"] & (df["Daily_Pct"] > 0)
    df["Loser"] = df["Active_Universe"] & df["Has_Prior_Close"] & (df["Daily_Pct"] < 0)

    df["Rolling_52W_High"] = group["High"].transform(lambda x: x.shift(1).rolling(252, min_periods=1).max())
    df["Rolling_52W_Low"] = group["Low"].transform(lambda x: x.shift(1).rolling(252, min_periods=1).min())
    df["Prior_ATH"] = group["High"].transform(lambda x: x.shift(1).cummax())
    df["New_52W_High"] = df["Active_Universe"] & (df["Prior_History_Days"] >= 252) & (df["Close"] >= df["Rolling_52W_High"])
    df["New_52W_Low"] = df["Active_Universe"] & (df["Prior_History_Days"] >= 252) & (df["Close"] <= df["Rolling_52W_Low"])
    df["IPO_New_High"] = df["Active_Universe"] & (df["Prior_History_Days"] >= 1) & (df["Prior_History_Days"] < 252) & (df["Close"] >= df["Prior_ATH"])
    df["Up_4_Pct"] = df["Active_Universe"] & (df["Daily_Pct"] >= 4.0)
    df["Down_4_Pct"] = df["Active_Universe"] & (df["Daily_Pct"] <= -4.0)
    df["Up_25_1M"] = df["Active_Universe"] & (df["Pct_1M"] >= 25.0)
    df["Down_25_1M"] = df["Active_Universe"] & (df["Pct_1M"] <= -25.0)
    ...
    df["TR"] = true_range
    df["ATR_14"] = group["TR"].transform(lambda x: x.ewm(alpha=1 / 14, min_periods=1, adjust=False).mean())
    df["Vol_20D_Avg"] = group["Volume"].transform(lambda x: x.shift(1).rolling(20, min_periods=1).mean())
    df["Max_20D_Prior"] = group["High"].transform(lambda x: x.shift(1).rolling(20, min_periods=1).max())
    ...
    prior_tight = group["VCP_Tightness"].shift(1).astype(float).fillna(0).astype(bool)
    df["Mature_Breakout"] = (df["Prior_History_Days"] >= 20) & (df["Close"] > df["Max_20D_Prior"])
    df["IPO_Breakout"] = (df["Prior_History_Days"] >= 1) & (df["Prior_History_Days"] < 20) & (df["Close"] > df["Listing_Day_High"])
    df["Is_Breakout"] = df["Active_Universe"] & (df["Mature_Breakout"] | df["IPO_Breakout"]) & df["Volume_Surge"] & prior_tight
    df["Is_Breakout_3d_ago"] = group["Is_Breakout"].shift(3).astype(float).fillna(0).astype(bool)
...
def liquidity_breadth(df: pd.DataFrame, name: str, prefix: str) -> pd.DataFrame:
    ...
    for period in [20, 50, 200]:
        result[f"{prefix}_Pct_{period}_EMA"] = np.where(result[f"Valid_{period}"] > 0, result[f"Above_{period}"] / result[f"Valid_{period}"] * 100, np.nan)
    return result[["Date", f"{prefix}_Pct_20_EMA", f"{prefix}_Pct_50_EMA", f"{prefix}_Pct_200_EMA"]]


def calculate_aggregate(stock: pd.DataFrame, previous: pd.DataFrame) -> pd.DataFrame:
    ...
    for period in [20, 50, 200]:
        result[f"Pct_Above_{period}_EMA"] = np.where(result[f"Valid_{period}"] > 0, result[f"Above_{period}_EMA"] / result[f"Valid_{period}"] * 100, np.nan)

    result["Rolling_3D_Up_4"] = result["Up_4_Count"].rolling(3).sum()
    result["Rolling_3D_Down_4"] = result["Down_4_Count"].rolling(3).sum()
    result["Net_52W_High_Low"] = result["New_52W_Highs"] - result["New_52W_Lows"]
    ...
    result["MCO"] = result["AD_Spread"].ewm(span=19, adjust=False).mean() - result["AD_Spread"].ewm(span=39, adjust=False).mean()
    result["TRIN"] = np.where((adv > 0) & (dec > 0) & (uv > 0) & (dv > 0), (adv / dec) / (uv / dv), np.nan)

    for name, prefix in [("Top 100 Liq", "Large"), ("Mid 150 Liq", "Mid"), ("Lower 250 Liq", "Small"), ("Micro Liq", "Micro")]:
        result = result.merge(liquidity_breadth(stock, name, prefix), on="Date", how="left")

    result = result.sort_values("Date").reset_index(drop=True)
    p_blend = 0.65 * result["Pct_Above_20_EMA"].fillna(0) + 0.35 * result["Pct_Above_50_EMA"].fillna(0)
    c1 = (p_blend / 100) * 25
    t3_b = result["T3_Breakouts"].astype(float)
    t3_w = result["T3_Wins"].astype(float)
    smoothed = (t3_w + 5) / (t3_b + 10)
    c2 = np.clip((smoothed - 0.45) / 0.15, 0, 1) * 25 * np.clip(t3_b / 10, 0, 1)
    net_4d = result["Rolling_3D_Up_4"] - result["Rolling_3D_Down_4"]
    net_1m = result["Up_25_1M_Count"] - result["Down_25_1M_Count"]
    ...
    rank_vol = result["Volume_Ratio"].fillna(1).rolling(126, min_periods=1).rank(pct=True)
    rank_hl = result["Net_52W_High_Low"].rolling(126, min_periods=1).rank(pct=True)
    c4 = np.where(result["Volume_Ratio"].fillna(0) > 1, rank_vol * 10, 0) + np.where(result["Net_52W_High_Low"] > 0, rank_hl * 10, 0)
    ...
    hunting = (result["Small_Pct_50_EMA"].fillna(0) + result["Micro_Pct_50_EMA"].fillna(0)) / 2
    c6 = -np.clip((result["Large_Pct_50_EMA"].fillna(0) - hunting - 20) * 0.75, 0, 15)
    c7 = np.where((result["Pct_Above_20_EMA"].rolling(20, min_periods=1).min() <= 10) & (p_blend >= 50), 15, 0)
    result["Composite_Score"] = (pd.Series(c1).fillna(0) + pd.Series(c2).fillna(0) + pd.Series(c3).fillna(0) + pd.Series(c4).fillna(0) + pd.Series(c5).fillna(0) + pd.Series(c6).fillna(0) + pd.Series(c7).fillna(0)).clip(0, 100).round().astype(int)
    result["Unchanged"] = result["Total_Universe"] - result["Advances"] - result["Declines"]
    return result.drop(columns=["Valid_20", "Valid_50", "Valid_200"])
...
def main() -> None:
    ...
    previous = pd.read_csv(AGGREGATE_FILE) if AGGREGATE_FILE.exists() else pd.DataFrame()
    ...
    atomic_write(stock_data[stock_data["Date"] >= TODAY - pd.Timedelta(days=450)], TRAILING_FILE)
    atomic_write(aggregate, AGGREGATE_FILE, csv=True)
    atomic_write(today_row[["Date", "Advances", "Declines", "Unchanged", "Total_Universe"]].assign(Time=NOW_IST.strftime("%H:%M")), BREADTH_FILE, csv=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Live analytics failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        sys.exit(1)
