...
def fetch_retry(fetcher, label: str):
    ...
    print(f"⚠️ {label} failed after {MAX_RETRIES} attempts: {last_error}")
    return None
...
def download_current_nse_eq_symbols() -> set[str]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            listed.columns = [str(column).strip().upper() for column in listed.columns]
            ...
            listed["SERIES"] = listed["SERIES"].astype(str).str.strip().str.upper()
            symbols = set(
                listed.loc[
                    (listed["SERIES"] == "EQ") & (listed["SYMBOL"] != ""),
                    "SYMBOL",
                ]
            )
            if len(symbols) < 1000:
                raise ValueError(
                    f"NSE current EQ universe looks incomplete: {len(symbols)} symbols."
                )
            return symbols
        except Exception as exc:
            last_error = exc
            print(
                f"⚠️ NSE EQUITY_L.csv download attempt {attempt + 1}/{MAX_RETRIES} "
                f"failed: {exc}"
            )
...
def confirm_nse_trading_session(kite: KiteConnect) -> None:
    ...
    if not nifty_data:
        fail("Could not retrieve NIFTY 50 quote to confirm the NSE trading session.")
    ...
    if not calendar:
        fail("Could not retrieve NIFTY trading calendar. Refusing to write intraday data.")
    ...
    if not market_dates:
        fail("NIFTY trading calendar returned no valid dates. Refusing to write intraday data.")

    if TODAY not in set(market_dates):
        skip(
            f"NSE is not trading on {TODAY.date()}. "
            f"Latest confirmed NIFTY market date: {market_dates[-1].date()}. "
            "Intraday update skipped safely."
        )
...
def fetch_live_quotes(
    ...
    def fetch_chunk(chunk: list[str], chunk_label: str) -> None:
        ...
        for key, data in response.items():
            symbol = clean_symbol(key.replace("NSE:", "", 1))
    ...
    for start in range(0, len(symbols), QUOTE_CHUNK_SIZE):
        ...
        fetch_chunk(chunk, str(start // QUOTE_CHUNK_SIZE + 1))
    ...
    invalid_symbols = set(
        live.loc[~live.apply(is_valid_live_ohlcv, axis=1), "Symbol"].map(clean_symbol)
    )
    live = live[live.apply(is_valid_live_ohlcv, axis=1)].copy()
    live["Date"] = pd.to_datetime(live["Date"]).dt.normalize()
    live = live.drop_duplicates(["Symbol", "Date"], keep="last")

    existing_failure_symbols = {record.get("Symbol") for record in failures}
    for symbol in sorted(invalid_symbols - existing_failure_symbols):
        failures.append({"Symbol": symbol, "Reason": "invalid_live_ohlcv"})
...
def prepare_stock_data(master: pd.DataFrame, live: pd.DataFrame) -> pd.DataFrame:
    ...
    for period in [20, 50, 200]:
        df[f"EMA_{period}"] = group["Close"].transform(
            lambda x, p=period: x.ewm(span=p, adjust=False, min_periods=1).mean()
        )
        df[f"Valid_{period}_EMA"] = (
            df["Active_Universe"] & (df["Prior_History_Days"] >= period)
        )
        df[f"Above_{period}_EMA"] = (
            df[f"Valid_{period}_EMA"] & (df["Close"] > df[f"EMA_{period}"])
        )
    ...
    df["ATR_14"] = group["TR"].transform(
        lambda x: x.ewm(alpha=1 / 14, min_periods=1, adjust=False).mean()
    )
    ...
    df["Follow_Through_Win"] = (
        df["Is_Breakout_3d_ago"] & (df["Close"] > df["Close_3d_ago"])
    )
...
def liquidity_breadth(df: pd.DataFrame, name: str, prefix: str) -> pd.DataFrame:
    ...
    for period in [20, 50, 200]:
        result[f"{prefix}_Pct_{period}_EMA"] = np.where(
            ...
            result[f"Above_{period}"] / result[f"Valid_{period}"] * 100,
            np.nan,
        )
    return result[
        [
...
def calculate_aggregate(stock: pd.DataFrame, previous: pd.DataFrame) -> pd.DataFrame:
    ...
    for period in [20, 50, 200]:
        result[f"Pct_Above_{period}_EMA"] = np.where(
            result[f"Valid_{period}"] > 0,
            result[f"Above_{period}_EMA"] / result[f"Valid_{period}"] * 100,
            np.nan,
        )
    ...
    c4 = (
        np.where(result["Volume_Ratio"].fillna(0) > 1, rank_vol * 10, 0)
        + np.where(result["Net_52W_High_Low"] > 0, rank_hl * 10, 0)
    )
    p200 = result["Pct_Above_200_EMA"].fillna(0)
    c5 = np.where(
        (p200 > 50) & (p200.diff(20).fillna(0) > 0),
        10,
        np.where((p200 <= 50) & (p200.diff(20).fillna(0) < 0), 0, 5),
    )
    hunting = (
    ...
    c6 = -np.clip(
        (result["Large_Pct_50_EMA"].fillna(0) - hunting - 20) * 0.75,
        0,
        15,
    )
    c7 = np.where(
        (result["Pct_Above_20_EMA"].rolling(20, min_periods=1).min() <= 10)
        & (p_blend >= 50),
        15,
        0,
    )
    ...
    result["Unchanged"] = (
        result["Total_Universe"] - result["Advances"] - result["Declines"]
    )
...
def main() -> None:
    ...
    current_eq_without_history = nse_eq_symbols - historical_symbols
    historical_not_current_eq = historical_symbols - nse_eq_symbols

    if len(required_symbols) < 1000:
        fail(
            "Intraday eligible universe is unexpectedly small after intersecting "
            "NSE current EQ symbols with the historical parquet."
        )

    critical_missing_from_universe = CRITICAL_SYMBOLS - required_symbols
    ...
    print(f"NSE current mainboard EQ symbols: {len(nse_eq_symbols)}")
    print(f"Intraday eligible symbols with usable history: {len(required_symbols)}")
    print(f"Current EQ symbols excluded because history is unavailable: {len(current_eq_without_history)}")
    print(f"Historical symbols excluded because they are no longer current EQ: {len(historical_not_current_eq)}")

    live, quote_failures = fetch_live_quotes(kite, sorted(required_symbols))
    live_symbols = set(live["Symbol"].map(clean_symbol))
    missing_live_symbols = required_symbols - live_symbols
    critical_missing_live = CRITICAL_SYMBOLS & missing_live_symbols
    coverage = len(live_symbols) / len(required_symbols)
    ...
    audit_records.extend(
        {
            "Symbol": symbol,
            "Date": TODAY,
            "Reason": "eligible_current_eq_missing_or_invalid_live_quote",
        }
    ...
    if audit_records:
        write_jsonl(AUDIT_DIR / f"intraday_coverage_{audit_stamp}.jsonl", audit_records)

    print(
        f"Live valid quote coverage: {len(live_symbols)}/{len(required_symbols)} "
        f"({coverage:.2%})."
    )
    ...
    if coverage < MIN_INTRADAY_COVERAGE:
        fail(
            f"Intraday coverage below required {MIN_INTRADAY_COVERAGE:.0%}: "
            f"{len(live_symbols)}/{len(required_symbols)} ({coverage:.2%})."
        )
    ...
    atomic_write(
        stock_data[stock_data["Date"] >= TODAY - pd.Timedelta(days=450)],
        TRAILING_FILE,
    )
    atomic_write(aggregate, AGGREGATE_FILE, csv=True)
    atomic_write(
        today_row[
            ["Date", "Advances", "Declines", "Unchanged", "Total_Universe"]
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"❌ Live analytics failed: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)
